...
class TestStrategy(bt.Strategy):
    ...
    def next(self, frompre=False):
        ...
        txt = ['Data0', '%04d' % len(self.data0), '{:f}'.format(self.data.datetime[0])]
        txt.extend(
            (
        ...
        if len(self.datas) > 1 and len(self.data1):
            dtfmt = '%Y-%m-%dT%H:%M:%S.%f'
            txt = ['Data1', '%04d' % len(self.data1), '{}'.format(self.data1.datetime[0])]
...
def runstrategy():
    ...
    if args.exactbars < 1 and args.plot:
